...
- 前置依赖：建议先理解 第 24 章：Slurm 与 HPC 调度 中的核心对象和路径。
- 后续关联：本章内容会继续连接到 第 26 章：裸金属 GPU 云，并在系统地图、深度标准和读者测试中被交叉引用。
- 读完能力：读完本章后，读者应能把《多集群与混部》中的概念映射到 AI Factory 的生产路径、工程对象、观测证据和设计取舍。


### 25.1.3 读者测试

- 机制题：读者能否解释 多集群管理、训练集群与推理集群、离线与在线混部、资源池化 的核心机制，以及它们如何共同支撑《多集群与混部》？
...
- 排障题：当《多集群与混部》相关生产症状出现时，读者能否列出第一层证据、下一跳证据、可能 owner 和止血动作？
...
![图：25.3.1 多集群管理](../assets/diagrams/part-05-orchestration-chapter-25-multi-cluster-02.svg)
...
![图：25.3.3 离线与在线混部](../assets/diagrams/part-05-orchestration-chapter-25-multi-cluster-03.svg)
