...
![图：32.2.2 系统架构](../assets/diagrams/part-07-network-storage-chapter-32-scale-out-networking-01.svg)

下面这张图强调多 rail 放置的证据链。设计上的 rail 只有在调度、容器、NCCL 和交换机端口计数都一致时才成立。某个环节漂移，都会让“多 rail 网络”退化成“看起来有多张网卡，实际流量集中在少数路径”。

![图：32.2.2 系统架构](../assets/diagrams/part-07-network-storage-chapter-32-scale-out-networking-02.svg)
...
RDMA 还会改变故障语义。普通 TCP 场景中，应用可能看到连接错误或明显超时；RDMA 场景中，错误可能表现为长时间等待、性能退化或少数队列异常。平台需要向用户呈现可理解的错误翻译，例如“容器未获得 RDMA 设备”“NCCL 未选择预期 GID”“某 rail 出现重传”。把底层计数直接甩给训练用户，无法形成有效运维闭环。
...
不同 collective 的风险不同。Data Parallel 的 AllReduce 或 ReduceScatter 往往规模大、周期稳定，适合用跨节点 fabric 扩展，但会被慢 rank 和 checkpoint 叠加放大。Tensor Parallel 如果跨节点，会把每层关键路径通信推到 scale-out fabric 上，通常只有在节点内 GPU 不足、网络等级很高且基线证明可接受时才应采用。Pipeline Parallel 的 P2P 流量更像相邻 stage 之间的定向通道，最好让相邻 stage 不跨过太远拓扑。Expert Parallel/MoE 的 AllToAll 受 token 分布影响，容易产生 rail、rack 或 expert 热点。Sequence/Context Parallel 的压力则取决于长序列实现和 attention 分块方式。
...
容量规划时还要区分“允许跨节点”和“建议跨节点”。DP 通常是 scale-out fabric 的主要消费方；PP 可以跨节点但要保护相邻 stage 的路径；EP 可以跨节点但要额外看 AllToAll 和负载均衡；TP 虽然技术上可以跨节点，工程上却最容易把网络推到关键路径，默认应先消耗 scale-up 域。这个区别应进入资源产品和调度模板，否则同样 256 张 GPU 会因为并行维度不同产生完全不同的网络压力。
...
![图：32.4.1 工程实现](../assets/diagrams/part-07-network-storage-chapter-32-scale-out-networking-03.svg)
