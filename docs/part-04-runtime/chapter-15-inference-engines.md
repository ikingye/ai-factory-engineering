...
Radix 或 prefix cache 的典型执行路径是：请求进入引擎后先用 tokenized prompt 匹配已有前缀，命中的部分作为缓存读 token，未命中的后缀进入 prefill 计算；新算出的、具备复用价值的前缀再写入缓存，形成缓存写 token。以 SGLang 为例，其 server 参数中 `--disable-radix-cache` 用于关闭 RadixAttention prefix caching，官方参数默认值为 `False`，也就是默认启用这类前缀缓存能力。这个默认值不代表所有请求都应该积极写入缓存，它只说明 runtime 具备前缀复用机制。
...
缓存空间来自有限的 KV cache/HBM 预算。上下文窗口配置很大，并不意味着历史请求都能永久缓存；当前请求的 KV、可复用前缀、allocator 碎片和其它租户都会竞争同一类显存资源。空间紧张时，runtime 会淘汰较旧、低复用或低价值缓存块。工程目标不是让 `cache_write_tokens` 越高越好，而是让后续 `cache_read_tokens` 提升，并让实际新算 prefill token 降低。

观测指标包括 cache hit rate、cache read tokens、cache write tokens、saved prefill tokens、cache memory、eviction、tenant/model/version 维度命中和命中后的 TTFT 改善。低命中率但高缓存占用说明策略不合适。Prefix cache 的目标是可证明地降低成本和延迟，而不是为了缓存而缓存。

多副本 serving 还需要 cache-aware 调度。若相似前缀请求被随机打到不同 worker，每个 worker 都要重复写入缓存，读命中率会被摊薄。SGLang Model Gateway 文档中的 `cache_aware` policy 就体现了这种思路：router 尽量在负载均衡和缓存局部性之间取舍，把相似请求送到更可能已有前缀缓存的 worker。平台应把这类策略视为路由与 runtime 的联合优化，而不是单纯 engine 内部特性。
...
| cache read/write conversion | 可优化 | 识别高写低读的低复用前缀和 cache pollution |
