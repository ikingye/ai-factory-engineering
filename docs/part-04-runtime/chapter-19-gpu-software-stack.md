...
flowchart TB
  ...
  Framework --> UserLibs["CUDA runtime / cuDNN / NCCL"]
  UserLibs --> Toolkit["NVIDIA Container Toolkit"]
  Toolkit --> Driver["NVIDIA Driver"]
  Driver --> GPU["GPU Hardware"]
  Driver --> DCGM["DCGM"]
  K8s["Kubernetes"] --> DevicePlugin["GPU Device Plugin"]
  K8s --> Operator["GPU Operator"]
  Operator --> Driver
  Operator --> Toolkit
  Operator --> DCGM
...
NVIDIA Container Toolkit 让容器能够访问 GPU 设备和必要的驱动库。它与 container runtime 集成，在容器启动时注入设备文件、driver libraries、环境变量和 capability，使容器内的 CUDA 应用可以使用主机 GPU。没有这层集成，Kubernetes 即使调度到了 GPU 节点，容器内应用也可能看不到 GPU。

Toolkit 问题常表现为边界错位。主机 `nvidia-smi` 正常，Kubernetes 也分配了 GPU，但容器内 `nvidia-smi` 失败，或应用报找不到 CUDA/driver library。排查路径应包括：Device Plugin 是否分配 GPU、runtime class 是否正确、Toolkit 是否安装、容器 runtime 配置是否生效、设备文件是否注入、驱动库挂载是否正确、容器权限是否满足。把这些问题笼统称为“GPU 不可用”会降低排障效率。

平台上，Container Toolkit 的版本应与 driver、container runtime、Kubernetes 版本和 GPU Operator 策略一起验证。升级 containerd、CRI-O、Docker 或节点 OS 时，也要验证 GPU 容器 smoke test。对于多租户环境，还要明确容器隔离边界：容器能看到哪些 GPU、哪些设备 capability 被开放、是否允许 privileged、是否暴露 RDMA 设备。Toolkit 是容器访问 GPU 的门闩，既影响可用性，也影响安全边界。

Toolkit 的验收应覆盖两类场景：最小容器只运行 `nvidia-smi` 和 CUDA sample，真实业务镜像加载框架并执行一次 GPU 计算。前者验证注入链路，后者验证用户态依赖组合。若只测 `nvidia-smi`，可能漏掉 CUDA library 或框架兼容问题；若只测业务镜像，问题又难以归因。分层测试能缩短排障路径。

此外，Toolkit 配置应纳入节点 drift 检测。container runtime 配置被手工修改后，节点应重新进入准入流程。
