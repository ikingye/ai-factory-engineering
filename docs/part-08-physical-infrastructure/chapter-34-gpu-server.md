...
```mermaid
flowchart TB
  BMC["BMC / Redfish"] --> Health["Hardware Events / Health State"]
  Power["Power Shelf / PSU"] --> Server["GPU Server"]
  Cooling["Fan / Liquid Cooling"] --> Server
  CPU["CPU / NUMA"] --- PCIe["PCIe Fabric"]
  PCIe --- GPU["GPU + HBM"]
  GPU --- NVLink["NVLink / NVSwitch Domain"]
  PCIe --- NIC["NIC / DPU"]
  PCIe --- NVMe["Local NVMe"]
  Health --> Pool["GPU Resource Pool"]
  Server --> Driver["Driver / Firmware"]
...
排查 CPU 相关问题时，应先把 GPU 等待拆开：是在等数据、等网络、等 kernel launch，还是等请求调度。常见证据包括 data loader 时间、tokenizer 延迟、CPU steal/load、NUMA remote access、网卡中断分布和进程绑核。没有这些证据，团队很容易把 CPU 路径问题误判为 GPU 算力不足。
...
PCIe 是服务器内部连接 CPU、GPU、NIC、DPU、NVMe 和其它设备的基础总线。即使节点有 NVLink 或 NVSwitch，GPU 到 CPU、GPU 到 NIC、GPU 到本地盘、DPU 到主机、部分 GPU 间路径仍可能经过 PCIe。PCIe 的 link width、link speed、root complex、switch 层级和 NUMA 位置，会影响数据进入和离开 GPU 的效率。
