...
如果你从应用侧进入，建议从第 1 章到第 8 章读起，再回到 Runtime 和调度层。如果你负责 GPU 集群或平台建设，建议从第 20 章开始读调度、GPU on Kubernetes、准入验收，再向上补 Platform 和模型服务。若你关心生产上线、商业化和成本，应把第 37-44 章作为第二入口：先看可观测、验收、故障诊断和 SRE，再看 Token Factory、商业模式、案例和建设路径。
