...
- 读者有效：章节写完后，要能通过 `doc-coauthoring` 式读者测试。也就是用没有上下文的新读者问题检验：读者是否能解释机制、边界、路径、落地动作和故障诊断，而不是只记住术语。
...
- 结构说清：每章使用 `H2 大组 + H3 小节` 的图书式层级。H2 固定为导读、基础模型、关键技术、工程落地、小结与延伸阅读；H3 使用 `章号.组号.小节号` 连续编号，包括上下文、读者测试、场景、概念、架构、主题技术节、工程实现、故障、指标、取舍、小结和延伸阅读。这样目录不会被二十多个同级条目淹没，正文引用、交叉审校和长期扩写仍使用稳定坐标。

## 2. 读者测试问题集

每次深度化一章或一条主题链路后，至少用以下问题类型做自测：

- 机制题：这个组件到底在控制面、数据面或运行时路径上做了什么？
- 边界题：它不负责什么？相邻组件失败时为什么不能归因到它？
- 路径题：一个请求、Pod、训练任务、checkpoint、GPU 设备或账单事件如何穿过系统？
- 排障题：出现典型症状时，第一层证据在哪里，下一跳证据是什么？
- 验收题：上线、扩容、升级或节点回池前，哪些基线必须通过？
- 经济题：这个机制如何影响 tokens/s、GPU hours、cost per token、SLA 或机会成本？
- 取舍题：为什么不是所有场景都选择同一种实现？
- 证据题：哪些结论需要官方文档、标准、论文或可公开验证案例支撑？
...
公开正文中的图统一引用 `docs/assets/diagrams/` 下的 SVG 资产。图源、SVG、PNG 和正文引用必须通过 `python3 tools/audit_diagrams.py` 校验；不再把 Mermaid 源直接暴露在章节正文中。
...
| `prefix_cache_accounting_record` | 第 1、7、15、37、41 章 | 说明 input token 不因 cache 命中扣减，cache read 才代表节省，cache write 是未来复用投资，高写低读意味着 cache pollution 风险 |
...
| scale-up topology contract | 第 31 章 | 说明 PCIe/NVLink 带宽方向、NVLink/NVSwitch 代际演进、GPU-to-GPU bandwidth、拓扑碎片和资源等级 |
...
| `gpu_interconnect_profile` | 第 31、34、35、38 章 | 说明 PCIe、NVLink/NVSwitch、NVLink-C2C、HGX/DGX/NVL72 等互联层级、带宽方向、代际演进和可调度 GPU 域 |
...
| `quality_gate_freshness_contract` | 第 13、37、39、41、44 章 | 说明一次 gate execution 在 dataset、judge、rubric、serving contract、route 和 feedback pipeline 依赖不变时才可复用，哪些变化会降级或失效 |
| `quality_evidence_dependency_graph` | 第 13、37、39、44 章 | 说明数据、judge、gate、serving contract、experiment、routing scorecard 和 PRR 的依赖关系如何查询影响面并传播失效 |
| `quality_evidence_invalidation_event` | 第 37、39、41、44 章 | 说明质量证据失效如何作为控制面事件冻结实验、阻断放量、重跑 gate、降级路由证据并打开成本记录 |
...
| `quality_evidence_validity` | 第 37、41、44 章 | 说明 quality evidence bundle 引用的 gate、污染记录、judge 校准和反馈 pipeline 是否仍可作为 PRR 与放量证据 |
| `quality_fault_tree_execution` | 第 39、44 章 | 说明质量事故如何按证据有效性、serving 组合、路由/实验、RAG/Agent、模型行为和经济影响执行故障树 |
| `quality_incident_cost_record` | 第 41、44 章 | 说明一次质量事故或质量证据失效如何把低质量 token、人工接管、重试、信用、gate rerun、judge calibration、阻断和避免损失写入经济账本 |
