...
一个客服 Chatbot 上线后，用户反馈“第一句话出来太慢”。后端 QPS 并不高，GPU 利用率也没有满，但请求日志显示很多 prompt 都带着长历史对话和 RAG 片段。推理服务的 prefill 队列被长上下文请求占住，短请求也被拖慢。

这个问题不是简单扩容就能解决。应用如何组织 message、是否开启 streaming、上下文长度如何限制、是否做 prefix cache，都会影响基础设施表现。

## 核心概念

Chat 请求是 AI Factory 中最常见的在线推理入口。它把用户体验、token 计量、模型服务、推理引擎和 GPU 显存连接起来。本章关注 message、prompt、context、input token、output token、streaming、TTFT、TPOT、prefill、decode 和 KV Cache 之间的关系。
...
这些字段用于关联用户体验、容量、计费和故障诊断。

## 常见故障

- 只看 QPS，不看 input/output token，导致容量估算错误。
- 长上下文请求和短请求混在同一队列，短请求 TTFT 被拖慢。
- Streaming 中断后没有记录已生成 token，计量和审计不完整。
- KV Cache 接近上限，但 dashboard 只展示 GPU compute utilization。

## 性能指标

- TTFT、TPOT、TPOP、E2E latency。
- input tokens/s、output tokens/s、平均和 P95 context length。
- KV Cache 使用率、batch size、队列等待时间。
- 错误率、取消率、streaming 中断率、限流率。

## 设计取舍

更长 context 提升能力，但增加 prefill 成本和 TTFT。更大 batch 降低 cost per token，但可能增加单请求延迟。更强 streaming 体验要求更复杂的连接管理、取消语义和计量策略。
