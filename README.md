...
这是一本面向工程师的开源技术书，系统讲解 AI Factory 如何从应用请求、MaaS 平台、模型服务、训练推理运行时、资源编排、GPU IaaS、网络存储、物理基础设施一路落到生产闭环和商业化度量。
...
- 生产闭环：可观测性、准入验收、故障诊断、SRE、Token Factory、商业模式、案例研究和从 0 到 1 建设路径。
- 横切能力：安全、成本、质量、可靠性和 Token Factory 经济性贯穿上述所有层次。
...
全书从“一个 Chat 请求如何变成 token”和“一个训练任务如何变成模型”两条主线展开，共 11 个 Part、45 章。前八个主体层次从应用一路下探到物理基础设施，最后两个 Part 收束为生产闭环：先证明系统可观测、可验收、可诊断、可运维，再讨论 token 经济、商业模式、案例和建设路径。当前仓库已经包含完整目录、写作规范、深度标准、术语表、路线图，以及第 0-44 章的系统化初稿。

当前写作阶段已经从“补齐章节”进入“深度化、图表化、证据化”的循环更新：每一轮围绕一条关键技术链路，补齐机制、架构图、配置、命令、故障树、指标、验收和跨章节覆盖。质量标准见 `docs/depth-standard.md`。
...
- 按 `docs/depth-standard.md` 补齐机制深度、故障树、验收和跨章节覆盖。
- 校对术语、图表和架构关系。
- 补充配置示例、排障流程、验收清单。
